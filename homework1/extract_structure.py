...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract structured requirement model from requirements text via Azure OpenAI."
    )
    parser.add_argument(
        "--requirements",
    ...
    parser.add_argument(
        "--output",
        default="output/structured_requirement.json",
...
def main():
    ...
    # Keep raw output if model accidentally returns non-JSON; fail fast for visibility.
    try:
    ...
    print(f"Structured requirement saved to: {output_path}")
